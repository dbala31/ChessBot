    ...
    <div className="p-4 lg:p-6">
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-2 text-sm" style={{ color: 'var(--text-muted)' }}>Loading games...</span>
        </div>
      )}

      {error && (
        <div className="flex items-center gap-2 rounded-lg px-4 py-3" style={{ background: 'var(--danger-light)', border: '1px solid var(--danger)' }}>
      ...
      {!loading && !error && games.length === 0 && (
        <div className="py-20 text-center">
          ...
          <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
            Go to <Link href="/settings" style={{ color: 'var(--accent)' }}>Settings</Link> to import games from Chess.com or Lichess.
          </p>
        </div>
      )}
            ...
            <tbody>
              {games.map((game) => (
                <tr key={game.id} className="cursor-pointer transition-colors duration-100" style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td className="px-4 py-3 font-medium" style={{ color: game.accuracy != null ? (game.accuracy >= 85 ? 'var(--success)' : game.accuracy >= 70 ? 'var(--warning)' : 'var(--danger)') : 'var(--text-muted)' }}>
                    {game.accuracy != null ? `${game.accuracy}%` : '\u2014'}
                  </td>
                  <td className="px-4 py-3" style={{ color: game.blunders != null ? (game.blunders === 0 ? 'var(--success)' : 'var(--danger)') : 'var(--text-muted)' }}>
