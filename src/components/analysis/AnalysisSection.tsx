...
export function AnalysisSection() {
  ...
  const [analysisResult, setAnalysisResult] = useState<{ completed: number; total: number } | null>(null)
  ...
  return (
    <div className="card p-5">
      ...
      {isAnalyzing && progress && (
        <div className="mb-3">
          <div className="mb-1.5 flex justify-between text-[11px]" style={{ color: 'var(--text-muted)' }}>
            <span>Game {progress.currentGame} of {progress.totalGames}</span>
            <span>Move {progress.currentPly} / {progress.totalPlies}</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full" style={{ background: 'var(--bg-tertiary)' }}>
            <div
              className="h-full rounded-full transition-all duration-300"
              style={{
                width: progress.totalPlies > 0
      ...
      {analysisError && (
        <p className="mb-3 text-xs" style={{ color: 'var(--danger)' }}>{analysisError}</p>
      )}
