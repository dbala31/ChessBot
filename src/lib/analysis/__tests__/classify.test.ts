...
// ── classifyMove (simple cpLoss-only version) ────────────────────────────────
...
describe('classifyMove', () => {
  ...
  it('classifies 10 cp loss as excellent', () => {
    expect(classifyMove(10)).toBe(MoveClassification.Excellent)
  })

  it('classifies 15 cp loss as excellent (upper boundary)', () => {
    expect(classifyMove(15)).toBe(MoveClassification.Excellent)
  })

  it('classifies 16 cp loss as good', () => {
    expect(classifyMove(16)).toBe(MoveClassification.Good)
  })
...
describe('determinePhase', () => {
  it('classifies starting position as opening', () => {
    expect(determinePhase('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', 1)).toBe(
      GamePhase.Opening,
    )
  })
  ...
  it('classifies early game (move 8) as opening', () => {
    expect(
      determinePhase('r1bqkb1r/pppppppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4', 8),
    ).toBe(GamePhase.Opening)
  })

  it('classifies move 20 with many pieces as middlegame', () => {
    expect(
      determinePhase('r1bq1rk1/pp2ppbp/2np1np1/8/3NP3/2N1BP2/PPPQ2PP/R3KB1R w KQ - 0 10', 20),
    ).toBe(GamePhase.Middlegame)
  })

  it('classifies position with few pieces as endgame', () => {
    expect(determinePhase('8/5pk1/8/8/8/8/5PK1/4R3 w - - 0 40', 80)).toBe(GamePhase.Endgame)
  })

  it('classifies king + pawns only as endgame', () => {
    expect(determinePhase('8/5pk1/6p1/8/8/6P1/5PK1/8 w - - 0 45', 90)).toBe(GamePhase.Endgame)
  })

  it('classifies queen + minor piece positions after move 15 as middlegame', () => {
    expect(
      determinePhase('r2q1rk1/ppp2ppp/2n2n2/3p4/3P4/2N2N2/PPP2PPP/R2Q1RK1 w - - 0 12', 24),
    ).toBe(GamePhase.Middlegame)
  })
})
